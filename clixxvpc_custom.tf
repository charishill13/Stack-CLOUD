...
resource "aws_security_group" "PUBSB" {
  name        = "PUBSB"
  description = "Allow SSH and HTTP"
  vpc_id     = aws_vpc.clixxappvpc.id
  ingress {
...
resource "aws_security_group" "loadbalancersgb" {
  name        = "loadbalancersgb"
  vpc_id     = aws_vpc.clixxappvpc.id
  ingress {
    description = "HTTP to load balancer"
    from_port   = 80
...
resource "aws_security_group" "bastionsga" {
  name        = "bastionsga "
  vpc_id     = aws_vpc.clixxappvpc.id
  description = "Allow SSH and HTTP"
  ingress {
...
resource "aws_security_group" "bastionsgb" {
  name        = "bastionsgb"
  vpc_id     = aws_vpc.clixxappvpc.id
  description = "Allow SSH and HTTP"
  ingress {
...
resource "aws_security_group" "apptordsa" {
  name        = "apptordsa"
  vpc_id     = aws_vpc.clixxappvpc.id
  ...
  ingress {
    ...
    to_port     = 80
    protocol    = "tcp"
    security_groups = [aws_security_group.PUBSGA.id,aws_security_group.PUBSB.id, aws_security_group.loadbalancersga.id,aws_security_group.loadbalancersgb.id]
  }
  ingress {
    description = "SSH from VPC"
    from_port   = 22
...
resource "aws_security_group" "rdstooraclea" {
  ...
  ingress {
    ...
    to_port     = 3306
    protocol    = "tcp"
    security_groups = [aws_security_group.apptordsa.id,aws_security_group.apptordsb.id]
 }
}
...
resource "aws_db_instance" "CustomClixxDB" {
  ...
  vpc_security_group_ids = [aws_security_group.apptordsa.id,aws_security_group.apptordsb.id,]
...
resource "aws_lb" "clixxapplb" {
  ...
  security_groups    = [aws_security_group.loadbalancersga.id,aws_security_group.loadbalancersgb.id]
